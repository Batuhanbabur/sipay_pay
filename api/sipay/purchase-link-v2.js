// api/sipay/purchase-link-v2.js  (Vercel Node.js Serverless Function - CommonJS)
module.exports = async (req, res) => {
  console.log('[purchase-link-v2] start', new Date().toISOString());
  ...
  try {
    if (!body || typeof body !== 'object') {
      // bazı ortamlarda req.body boş olabilir → ham gövdeyi oku
    ...
    console.error('[purchase-link-v2] BAD_JSON', e?.message);
  ...
  try {
    ...
    if (!ctype.includes('application/json')) {
      ...
      return res.status(502).json({
        ok:false, error:'SIPAY_NON_JSON',
        status:r.status, contentType:ctype,
    ...
    if (data && (data.link || data.order_id || data.status)) {
      console.log('[purchase-link-v2] OK', data.order_id || data.link);
