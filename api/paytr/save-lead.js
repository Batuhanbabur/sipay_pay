// api/paytr/save-lead.js
// "GÜVENLİ ÖDE" butonuna basılınca çağrılır.
// Paralel olarak:
//   1. Google Sheets'e satır ekler (Apps Script webhook)
//   2. Brevo ile sana bildirim e-postası gönderir

export default async function handler(req, res) {
  const origin = req.headers.origin || "";
  const allowed = ["https://do-lab.co","https://www.do-lab.co"];
  if(allowed.includes(origin)) res.setHeader("Access-Control-Allow-Origin", origin);
  else res.setHeader("Access-Control-Allow-Origin", "https://do-lab.co");
  ...
  const NOTIFY_EMAIL   = process.env.NOTIFY_EMAIL;   // sana gelecek: batuhan@do-lab.co
  const FROM_EMAIL     = process.env.FROM_EMAIL;     // doğrulanmış: orders@do-lab.co
